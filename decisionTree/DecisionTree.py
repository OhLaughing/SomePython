...
def plotTree(myTree, parentPt, nodeTxt, allrelations, features):
    ...
    # 下个字典
    index = next(iter(myTree))
    firstStr = features[index]
    # 中心位置
    ...
    for key in secondDict.keys():
        ...
        if type(secondDict[key]).__name__ == 'dict':
            # 不是叶结点，递归调用继续绘制
            plotTree(secondDict[key], cntrPt, allrelations[index][int(key)], allrelations, features)
...
def createPlot(inTree, allrelation, features):
    # 创建fig
    ...
    plotTree.yoff = 1.0
    # 绘制决策树
    plotTree(inTree, (0.5, 1.0), '', allrelation, features)
    # 显示绘制结果
    plt.show()
